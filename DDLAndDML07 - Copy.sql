...
-------Having Clouse
select (id),COUNT(id) as 'count' from Employye group by id
select * from Employye
select (city),count (city) as 'count' from Employye group by city

select (id),sum (salary) as 'sum' from Employye group by id
